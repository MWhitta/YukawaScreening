...
theory makes a parameter-free prediction for how rapidly bond valence
should decay with distance once two chemically transparent quantities
are fixed: the cation valence and the number of neighbors in its
coordination shell.  That prediction is confirmed by 150 fitted slopes
from 94 cation--oxygen species spanning the $s$, $d$, $f$, and $p$
blocks, with formal charges $+1$ through $+7$. The same shell-scale
picture is confirmed against
...
Third,
the work predicts a sharp crossover between screening and anti-screening
when a cation's formal charge is distributed as one valence unit per
bond.  That crossover gives the fitted parameters a direct meaning as a
measure of local charge-transfer response: whether the surrounding
oxygen network reduces or reverses the cation's effective electrostatic
field.  The resulting tabulated screening lengths
organize bond-valence thermodynamics across diverse cation--oxygen
...
descriptor of local charge transfer.  A fully packaged code
repository for independent validation and refitting is available at
\url{https://github.com/MWhitta/YukawaScreening}.
...
\textbf{Data availability:} All crystal structures used in this work
are publicly available from the Materials Project
(\url{https://materialsproject.org}).  The fitted bond-valence
parameters, characteristic pairs $(R_0^*,\,B^*)$ for all 103
cation--oxygen species and physical screening lengths~$\lambda^*$ for
the 101 screening-branch species are tabulated in the Supplemental
Material; the two anti-screening cases are retained there as signed
diagnostics.  The analysis code, the bond-valence summary records, and
a Materials Project ID manifest for the charge-density inputs are
...
equations) and fits within four pages.
