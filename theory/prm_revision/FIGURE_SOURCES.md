...
| --- | --- | --- |
| `prm_oxygen_beta_vs_charge_box_cn4_cn6.png` | `data/processed/bond_valence/consolidated_store.json::oxygen_authoritative` + `data/processed/theory/master_oxygen_summary_theory.json` | `PYTHONPATH=analysis python analysis/scripts/make_dblock_beta_prl_figure.py` |
...
| `lambda_minus_vs_weighted_radius.png` | `theory/prm_revision/si_lambda_table.tex` + `theory/exploratory/species_weighted_radii.json` (Shannon 1976 radii via pymatgen, `theory/exploratory/shannon_radii_full.json`; CN populations from `data/processed/bond_valence/consolidated_store.json::oxygen_authoritative`) | `python theory/exploratory/build_weighted_radii.py && python theory/exploratory/lambda_minus_vs_shannon.py`, then copy PNG to `theory/figures/` |
